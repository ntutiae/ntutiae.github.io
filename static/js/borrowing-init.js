...
export function webInit(backMonth, forwardMonth, refresh, month) {
  /* 生成前一月按鈕 */
  const backBtn = elementManager.createElement({
    tag: 'button',
    onclick: backMonth,
    childs: [
      // 子元素 => icon
  ...
  /* 生成目前檢視月份元素 */
  const textBase = elementManager.createElement({
    tag: 'span',
    innerHTML: `${month} 月`,
  })

  /* 生成後一月按鈕 */
  const forwardBtn = elementManager.createElement({
    tag: 'button',
  ...
  /* 生成刷新按鈕 */
  const refreshBtn = elementManager.createElement({
    tag: 'button',
    onclick: refresh,
    classes: ['refresh-btn'],
    appendTo: header,
