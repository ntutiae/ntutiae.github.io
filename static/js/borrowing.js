...
;(async () => {
  ...
  let calendarData = await request.GET(
  ...
  /* 過濾出所有不可借用日期 ex. 
    ...
    */
  const holiday = calendarData.filter(
    (d) => d.isHoliday || d.week === '五' || d.week === '六' || d.week === '日'
  )
  ...
  /* 取得API(借用紀錄 試算表) 資料 ex.
    ...
    */
  const record = await request.GET(API, null, true)
...
function main(holiday, record) {
  ...
  const isHoliday = (dateStr) => holiday.some((d) => d.date === dateStr)
  const getReason = (dateStr) =>
    holiday.filter((d) => d.date === dateStr)[0].description
  ...
  const buttonChange = () => {
    ...
    if (month % 12 === (new Date().getMonth() + 3) % 12)
  ...
  const getStatus = (dateStr, timeStr) => {
    /* 過濾出指定時段的借用紀錄資料 ex.
        ...
        */
    const dat = record.filter(
      (e) => e[0] === dateStr && e[1].startsWith(timeStr)
    )

    if (!dat.length) return ['idle', '閒置'] // 指定時段沒有借用紀錄資料

    if (Number(dateStr) < Number(dateUtil.getDateStr())) {
      // 如果日期已過
      if (timeStr.startsWith('晚上')) {
        // 如果時段在晚上
        if (!(dat[0][2] && dat[0][3] && dat[0][5])) return ['idle', '閒置'] // 沒借到教室
        dat[0][4] ? ['complate', '完成'] : ['waiting', '缺照片'] // 三個勾選框都有勾(有借到教室)
      }
    ...
    if (timeStr.startsWith('晚上'))
      return dat[0][2] && dat[0][3] && dat[0][5]
        ? ['used', '已占用']
        : ['apply', '審核中'] // 檢驗三個勾選框是否都勾
  ...
  const createStatus = (dateStr, timeStr, urlDateStr, dateBase) => {
    /* 獲取指定日期、時段狀態 
        ...
        */
    const status = getStatus(dateStr, timeStr.slice(0, 2))
  ...
  const dateChange = async (loading, doNotRefresh = null) => {
    ...
    /* 取得此月的日曆 ex.
            ...
            (兩個 null 填充前面兩個空日期)
        */
    const monthDates = dateUtil.getDaysInMonth(year, month)
    for (const monthDate of monthDates) {
      // 尋一遍日期 null, null, 1, 2, 3......

      /* 獲取日期字串 dateStr => YYYYMMDD */
      const dateStr = dateUtil?.getDateStr(
        dateUtil.getDateFromNumber(year, month, monthDate)
      )

      /* 生成日期元素 */
      const dateBase = elementManager.createElement({
        tag: 'span',
        classes: [
          'date',
      ...
      const urlDateStr = dateUtil.getDateStr(
        dateUtil.getDateFromNumber(year, month, monthDate),
        '-'
      )
  ...
  const refresh = async () => {
    ...
    // 將日曆換上 Loading... 圖示
    const loading = elementManager.createElement({
      tag: 'span',
      classes: ['foot'],
      childs: [
        elementManager.createElement({
  ...
  /* 初始化日曆 (生成各種按鈕及元素) borrowing-init.js Line. 18
        ...
        textBase   => 檢視月份元素
    */
  let { forwardBtn, backBtn, refreshBtn, textBase } = webInit(
    backMonth,
    forwardMonth,
    refresh,
    month
